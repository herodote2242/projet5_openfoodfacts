...
class ProductManager:
    ...
    def find_n_unhealthy_products_by_category(self, category, n=c.NUMBER_OF_BAD_FOOD):

        """This function searches in the database'table "product" n random products
        of the category selected with a bad nutriscore."""
        unhealthy_result = self.db.query(f"""SELECT DISTINCT product_name FROM product
            ...
            catname=c.CATEGORIES_TO_RECOVER[category_selected], n=n)
        return unhealthy_result.all(as_dict=True)

    def find_n_healthy_products_by_category(self, category, n=c.NUMBER_OF_GOOD_FOOD):
        """This function searches in the database'table "product" n random products
        of the category selected with a good nutriscore."""
    ...
    def find_product_description(self):
        ...
        description = self.db.query(f"""SELECT * FROM product WHERE product.code = :code""",
            code=product_code)
        return description

class StoreManager:
    """ This class contains the function related to the actions the user can ask for 
    related to the table 'store'."""
    def __init__(self, db, connection):
        self.db = db
        self.connection = connection

    def find_stores_by_product_code(self, product_code):
        # The function is called when the user wants to know where he can buy a product.
...
class FavoriteManager:
    ...
    def find_favorite_list(self):
        # The function displays all the favorites saved in the table and their names.
    ...
    def add_favorite_from_product_code(self, product_code):
        # The function is called when the user wants to add a product into the table favorite.
        self.db.query(f"""INSERT INTO favorite VALUES :product_code""", product_code=product_code)

    def delete_from_favorite(self, product_code):
        # The function is called when the user wants to delete one of the favorites.
        self.db.query(f"""DELETE FROM favorite WHERE product_id = :product_code""", product_code=product_code)

    def find_favorite_description(self, product_code):
        """This function shows the description of a selected product."""
        description = self.db.query(f"""SELECT * FROM product JOIN favorite
            ON product.code = favorite.product_id""", product_code=product_code)
        return description
